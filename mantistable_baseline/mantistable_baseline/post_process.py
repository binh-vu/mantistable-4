...
def get_cpa(method, col_tags, linkage) -> List[Tuple[int, int, str]]:
...
def get_cta(
    ...
    for qnode_id in selected_qnode_ids:
        if qnode_id not in qnodes:
            print(
                f">>> [CTA] - [missing entity error] skip entity {qnode_id} is not in the databases"
            )
            continue
        qnode = qnodes[qnode_id]
        concepts = {stmt.value.as_entity_id() for stmt in qnode.props.get("P31", [])}
    ...
    for input in inputs:
        ...
        for real_ci in cta_cols:
            ...
            if len(global_concepts) == 0:
                continue
            if input.table.table_id not in cta:
                cta[input.table.table_id] = {}
            cta[input.table.table_id][real_ci] = global_concepts
